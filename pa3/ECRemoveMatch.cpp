...
void removeMatch(vector<string *> &string_pointers, string *to_remove){
	...
	for(size_t i = 0; i < string_pointers.size(); i++){
		if(string_pointers[i] == to_remove){
			//remove string
			string_pointers.erase(string_pointers.begin() + i);
			//decrement i since the array will be shifted by erase
			i--;
		}
	}
